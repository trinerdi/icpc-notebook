 ...
 *  Initialize the tree with {\tt create(patterns)}. {\tt find(word)} returns for each position
 *  the index of the longest word that ends there, or -1 if none. {\tt findAll(\_, word)} finds all words
 ...
 *  find is $O(M)$ where $M$ is the length of the word. {\tt findAll} is $O(NM)$.
