 ...
 * Description: Min-cost max-flow. $cap[i][j] \neq cap[j][i]$ is allowed; double edges are not.
 *  If costs can be negative, call {\tt setpi} before maxflow, but note
 *  that negative cost cycles are not allowed (that's NP-hard).
