...
int main() {
    ...
    REP(i,N) { SUF[i].idx = i; SUF[i].por[0] = s[i]; }   // inicializacia pola suffixov (tady bylo s[i]-'a')
     
    int sl = 1;    // sl je dlzka prefixu, podla ktoreho su aktualne zotriedene
    do {
        REP(i,N) pom[ SUF[i].idx ] = SUF[i].por[0];   // naplnime pomocne pole aktualnymi poziciami suffixov

        REP(i,N) {      // vypocitame druhu zlozku kluca pre kazdy suffix
